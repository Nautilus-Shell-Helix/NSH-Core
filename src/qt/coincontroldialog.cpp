...
QString CoinControlDialog::getPriorityLabel(double dPriority, double mempoolEnshcatePriority)
{
    double dPriorityMedium = mempoolEnshcatePriority;
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        // Priority
        double mempoolEnshcatePriority = mempool.enshcateSmartPriority(nTxConfirmTarget);
        dPriority = dPriorityInputs / (nBytes - nBytesInputs + (nQuantityUncompressed * 29)); // 29 = 180 - 151 (uncompressed public keys are over the limit. max 151 bytes of the input are ignored for priority)
        sPriorityLabel = CoinControlDialog::getPriorityLabel(dPriority, mempoolEnshcatePriority);
        ...
        // Allow free? (require at least hard-coded threshold and default to that if no enshcate)
        double dPriorityNeeded = std::max(mempoolEnshcatePriority, AllowFreeThreshold());
    ...
    int nDisplayUnit = BitcoinUnits::NSHC;
    ...
    // how many satoshis the enshcated fee can vary per byte we guess wrong
    double dFeeVary;
    if (payTxFee.GetFeePerK() > 0)
        dFeeVary = (double)std::max(CWallet::GetRequiredFee(1000), payTxFee.GetFeePerK()) / 1000;
    else {
        dFeeVary = (double)std::max(CWallet::GetRequiredFee(1000), mempool.enshcateSmartFee(nTxConfirmTarget).GetFeePerK()) / 1000;
    }
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            if(ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show nshc address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            itemOutput->setText(COLUMN_PRIORITY, CoinControlDialog::getPriorityLabel(dPriority, mempoolEnshcatePriority));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_PRIORITY, CoinControlDialog::getPriorityLabel(dPrioritySum, mempoolEnshcatePriority));
