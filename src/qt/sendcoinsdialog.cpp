...
void SendCoinsDialog::updateFeeSectionControls()
{
    ...
    ui->labelFeeEnshcation      ->setEnabled(ui->radioSmartFee->isChecked());
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    int enshcateFoundAtBlocks = nBlocksToConfirm;
    CFeeRate feeRate = mempool.enshcateSmartFee(nBlocksToConfirm, &enshcateFoundAtBlocks);
    if (feeRate <= CFeeRate(0)) // not enough data => minfee
    {
        ui->labelSmartFee->setText(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
                                                                std::max(CWallet::fallbackFee.GetFeePerK(), CWallet::GetRequiredFee(1000))) + "/kB");
        ui->labelSmartFee2->show(); // (Smart fee not initialized yet. This usually takes a few blocks...)
        ui->labelFeeEnshcation->setText("");
    }
    else
    {
        ui->labelSmartFee->setText(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
        ...
        ui->labelFeeEnshcation->setText(tr("Enshcated to begin confirmation within %n block(s).", "", enshcateFoundAtBlocks));
    }
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid NSHCoin address"));
        }
        else // Valid address
        {
            CKeyID keyid;
