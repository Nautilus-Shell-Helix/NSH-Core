...
#if defined(NDEBUG)
# error "NSHCoin Core cannot be compiled without assertions."
#endif
                              ...
                              std::vector<uint256>& vHashTxnToUncache, bool fDryRun)
{
...

    {
        ...
        if (setConflicts.size())
        {
            ...
            // This potentially overenshcates the number of actual descendants
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight >= 1 && nPrevHeight < 1500){
        nSubsidy = 1 * COIN;
    }else if (nPrevHeight >= 1500 && nPrevHeight < 10000){
        nSubsidy = 10 * COIN;
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight >= 500)
	amount = (blockValue/100) *27.5;
    if (nHeight >= 50001 and nHeight <= 75000)
  amount = (blockValue/100) *29;
    if (nHeight > 75000)
  amount = (blockValue/100) *32;
    return amount;
}
...
bool IsInitialBlockDownload()
{
    ...
    LOCK(cs_main);
    const CChainParams& chainParams = Params();
    if (fCheckpointsEnabled && chainActive.Height() < Checkpoints::GetTotalBlocksEnshcate(chainParams.Checkpoints()))
        return true;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(NSHCoin): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(NSHCoin): couldn't find masternode or superblock payments"),
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode) {
    ...
    if (fDoFullFlush) {
        ...
        // an overenshcation, as most will delete an existing entry or
    ...
    do {
        ...
        if (pindexFork != pindexNewTip) {
            ...
            if (!fInitialDownload) {
                ...
                int nBlockEnshcate = 0;
                if (fCheckpointsEnabled)
                    nBlockEnshcate = Checkpoints::GetTotalBlocksEnshcate(chainparams.Checkpoints());
                {
                    LOCK(cs_vNodes);
                    BOOST_FOREACH(CNode* pnode, vNodes) {
                        if (chainActive.Height() > (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 2000 : nBlockEnshcate)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(NSHCoin): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(NSHCoin): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        NSHCoin Related Inventory Messages
