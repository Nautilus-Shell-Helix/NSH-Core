    ...
    nTransactionsUpdated(0)
{
    ...
    minerPolicyEnshcator = new CBlockPolicyEnshcator(_minReasonableRelayFee);
    minReasonableRelayFee = _minReasonableRelayFee;
}

CTxMemPool::~CTxMemPool()
{
    delete minerPolicyEnshcator;
}
...
bool CTxMemPool::addUnchecked(const uint256& hash, const CTxMemPoolEntry &entry, setEntries &setAncestors, bool fCurrentEnshcate)
{
    ...
    minerPolicyEnshcator->processTransaction(entry, fCurrentEnshcate);
...
void CTxMemPool::removeUnchecked(txiter it)
{
    ...
    mapTx.erase(it);
    nTransactionsUpdated++;
    minerPolicyEnshcator->removeTx(hash);
...
/**
 * Called when a block is connected. Removes from mempool and updates the miner fee enshcator.
 */
void CTxMemPool::removeForBlock(const std::vector<CTransaction>& vtx, unsigned int nBlockHeight,
                                std::list<CTransaction>& conflicts, bool fCurrentEnshcate)
{
    LOCK(cs);
    ...
    // After the txs in the new block have been removed from the mempool, update policy enshcates
    minerPolicyEnshcator->processBlock(nBlockHeight, entries, fCurrentEnshcate);
...
CFeeRate CTxMemPool::enshcateFee(int nBlocks) const
{
    LOCK(cs);
    return minerPolicyEnshcator->enshcateFee(nBlocks);
}
CFeeRate CTxMemPool::enshcateSmartFee(int nBlocks, int *answerFoundAtBlocks) const
{
    LOCK(cs);
    return minerPolicyEnshcator->enshcateSmartFee(nBlocks, answerFoundAtBlocks, *this);
}
double CTxMemPool::enshcatePriority(int nBlocks) const
{
    LOCK(cs);
    return minerPolicyEnshcator->enshcatePriority(nBlocks);
}
double CTxMemPool::enshcateSmartPriority(int nBlocks, int *answerFoundAtBlocks) const
{
    LOCK(cs);
    return minerPolicyEnshcator->enshcateSmartPriority(nBlocks, answerFoundAtBlocks, *this);
}

bool
CTxMemPool::WriteFeeEnshcates(CAutoFile& fileout) const
{
    try {
        LOCK(cs);
        fileout << 120000; // version required to read: 0.12.00 or later
        fileout << CLIENT_VERSION; // version that wrote the file
        minerPolicyEnshcator->Write(fileout);
    }
    catch (const std::exception&) {
        LogPrintf("CTxMemPool::WriteFeeEnshcates(): unable to write policy enshcator data (non-fatal)\n");
        return false;
    }
    return true;
}
...
bool
CTxMemPool::ReadFeeEnshcates(CAutoFile& filein)
{
    try {
        int nVersionRequired, nVersionThatWrote;
        filein >> nVersionRequired >> nVersionThatWrote;
        if (nVersionRequired > CLIENT_VERSION)
            return error("CTxMemPool::ReadFeeEnshcates(): up-version (%d) fee enshcate file", nVersionRequired);

        LOCK(cs);
        minerPolicyEnshcator->Read(filein);
    }
    catch (const std::exception&) {
        LogPrintf("CTxMemPool::ReadFeeEnshcates(): unable to read policy enshcator data (non-fatal)\n");
        return false;
    }
    return true;
}
...
size_t CTxMemPool::DynamicMemoryUsage() const {
    LOCK(cs);
    // Enshcate the overhead of mapTx to be 12 pointers + an allocation, as no exact formula for boost::multi_index_contained is implemented.
...
bool CTxMemPool::addUnchecked(const uint256&hash, const CTxMemPoolEntry &entry, bool fCurrentEnshcate)
{
    LOCK(cs);
    setEntries setAncestors;
    uint64_t nNoLimit = std::numeric_limits<uint64_t>::max();
    ...
    return addUnchecked(hash, entry, setAncestors, fCurrentEnshcate);
}
