...
/** \class CBlockPolicyEnshcator
 * The BlockPolicyEnshcator is used for enshcating the fee or priority needed
 ...
 * decide whether it can be used as a data point for fee enshcation, priority
 * enshcation or neither.  If the value of exactly one of those properties was
 * below the required minimum it can be used to enshcate the other.  In
 * addition, if a priori our enshcation code would indicate that the
 ...
 * an enshcate for that property.
 *
 * Here is a brief description of the implementation for fee enshcation.
 * When a transaction that counts for fee enshcation enters the mempool, we
 ...
 * an enshcate for any number of confirmations below the number of blocks
...
public:
    /**
     * Initialize the data structures.  This is called by BlockPolicyEnshcator's
    ...
    /** Update our enshcates by decaying our historical moving average and updating
        with the data gathered from the current block */
    void UpdateMovingAverages();

    /**
     * Calculate a fee or priority enshcate.  Find the lowest value bucket (or range of buckets
     ...
     */
    double EnshcateMedianVal(int confTarget, double sufficientTxVal,
                             double minSuccess, bool requireGreater, unsigned int nBlockHeight);

    /** Return the max number of confirms we're tracking */
    unsigned int GetMaxConfirms() { return confAvg.size(); }

    /** Write state of enshcation data to a file*/
    void Write(CAutoFile& fileout);

    /**
     * Read saved state of enshcation data from a file and replace all internal data structures and
...
/** Track confirm delays up to 25 blocks, can't enshcate beyond that */
...
// to give accurate enshcates over a large range of potential fees and priorities
...
/**
 *  We want to be able to enshcate fees or priorities that are needed on tx's to be included in
...
{
public:
    /** Create new BlockPolicyEnshcator and initialize stats tracking classes with default values */
    CBlockPolicyEnshcator(const CFeeRate& minRelayFee);
    ...
    void processTransaction(const CTxMemPoolEntry& entry, bool fCurrentEnshcate);
    ...
    /** Return a fee enshcate */
    CFeeRate enshcateFee(int confTarget);

    /** Enshcate fee rate needed to get be included in a block within
     *  confTarget blocks. If no answer can be given at confTarget, return an
     *  enshcate at the lowest target where one can be given.
     */
    CFeeRate enshcateSmartFee(int confTarget, int *answerFoundAtTarget, const CTxMemPool& pool);

    /** Return a priority enshcate */
    double enshcatePriority(int confTarget);

    /** Enshcate priority needed to get be included in a block within
     *  confTarget blocks. If no answer can be given at confTarget, return an
     *  enshcate at the lowest target where one can be given.
     */
    double enshcateSmartPriority(int confTarget, int *answerFoundAtTarget, const CTxMemPool& pool);

    /** Write enshcation data to a file */
    void Write(CAutoFile& fileout);

    /** Read enshcation data from a file */
    void Read(CAutoFile& filein);
