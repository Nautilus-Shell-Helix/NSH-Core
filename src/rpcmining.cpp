 ...
 * If 'height' is nonnegative, compute the enshcate at the time when a given block was found.
 */
...
UniValue getnetworkhashps(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nReturns the enshcated network hashes per second based on the last n blocks.\n"
            "Pass in [blocks] to override # of blocks, -1 specifies since last difficulty change.\n"
            "Pass in [height] to enshcate the network speed at the time when a certain block was found.\n"
            "\nArguments:\n"
            "1. blocks     (numeric, optional, default=120) The number of blocks, or -1 for blocks since last difficulty change.\n"
            "2. height     (numeric, optional, default=-1) To enshcate at the time of the given height.\n"
            "\nResult:\n"
            "x             (numeric) Hashes per second enshcated\n"
            "\nExamples:\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "NSHCoin Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "NSHCoin Core is downloading blocks...");

    if (!masternodeSync.IsSynced())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "NSHCoin Core is syncing with network...");
...
UniValue enshcatefee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "enshcatefee nblocks\n"
            "\nEnshcates the approximate fee per kilobyte needed for a transaction to begin\n"
            "confirmation within nblocks blocks.\n"
            "\nArguments:\n"
            "1. nblocks     (numeric)\n"
            "\nResult:\n"
            "n              (numeric) enshcated fee-per-kilobyte\n"
            "\n"
            "A negative value is returned if not enough transactions and blocks\n"
            "have been observed to make an enshcate.\n"
            "\nExample:\n"
            + HelpExampleCli("enshcatefee", "6")
            );
    ...
    CFeeRate feeRate = mempool.enshcateFee(nBlocks);
...
UniValue enshcatepriority(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "enshcatepriority nblocks\n"
            "\nEnshcates the approximate priority a zero-fee transaction needs to begin\n"
            "confirmation within nblocks blocks.\n"
            "\nArguments:\n"
            "1. nblocks     (numeric)\n"
            "\nResult:\n"
            "n              (numeric) enshcated priority\n"
            "\n"
            "A negative value is returned if not enough transactions and blocks\n"
            "have been observed to make an enshcate.\n"
            "\nExample:\n"
            + HelpExampleCli("enshcatepriority", "6")
            );
    ...
    return mempool.enshcatePriority(nBlocks);
}

UniValue enshcatesmartfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "enshcatesmartfee nblocks\n"
            "\nWARNING: This interface is unstable and may disappear or change!\n"
            "\nEnshcates the approximate fee per kilobyte needed for a transaction to begin\n"
            "confirmation within nblocks blocks if possible and return the number of blocks\n"
            "for which the enshcate is valid.\n"
            "\nArguments:\n"
            "1. nblocks     (numeric)\n"
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) enshcate fee-per-kilobyte (in BTC)\n"
            "  \"blocks\" : n         (numeric) block number where enshcate was found\n"
            "}\n"
            "\n"
            "A negative value is returned if not enough transactions and blocks\n"
            "have been observed to make an enshcate for any number of blocks.\n"
            "However it will not return a value below the mempool reject fee.\n"
            "\nExample:\n"
            + HelpExampleCli("enshcatesmartfee", "6")
            );
    ...
    UniValue result(UniValue::VOBJ);
    int answerFound;
    CFeeRate feeRate = mempool.enshcateSmartFee(nBlocks, &answerFound);
    result.push_back(Pair("feerate", feeRate == CFeeRate(0) ? -1.0 : ValueFromAmount(feeRate.GetFeePerK())));
    result.push_back(Pair("blocks", answerFound));
    return result;
}

UniValue enshcatesmartpriority(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "enshcatesmartpriority nblocks\n"
            "\nWARNING: This interface is unstable and may disappear or change!\n"
            "\nEnshcates the approximate priority a zero-fee transaction needs to begin\n"
            "confirmation within nblocks blocks if possible and return the number of blocks\n"
            "for which the enshcate is valid.\n"
            ...
            "\nResult:\n"
            "{\n"
            "  \"priority\" : x.x,    (numeric) enshcated priority\n"
            "  \"blocks\" : n         (numeric) block number where enshcate was found\n"
            "}\n"
            "\n"
            "A negative value is returned if not enough transactions and blocks\n"
            ...
            "\nExample:\n"
            + HelpExampleCli("enshcatesmartpriority", "6")
            );
    ...
    UniValue result(UniValue::VOBJ);
    int answerFound;
    double priority = mempool.enshcateSmartPriority(nBlocks, &answerFound);
