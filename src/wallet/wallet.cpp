...
/**
 * If fee enshcation does not have enough data to provide enshcates, use this fee instead.
 * Has no effect if not using fee enshcation
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 NSHCoin.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 1000 NSHCoin.");
                    ...
                    if (fUseInstantSend) {
                        if (nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                            strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 NSHCoin."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                        } else {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-nshc-address
                        CScript scriptChange;
                ...
                *static_cast<CTransaction*>(&wtxNew) = CTransaction(txNew);
                ...
                if (fSendFreeTransactions && nBytes <= MAX_FREE_TRANSACTION_CREATE_SIZE)
                {
                    ...
                    double dPriorityNeeded = mempool.enshcateSmartPriority(nTxConfirmTarget);
...
CAmount CWallet::GetMinimumFee(unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool)
{
    ...
    // User didn't set: use -txconfirmtarget to enshcate...
    if (nFeeNeeded == 0) {
        int enshcateFoundTarget = nConfirmTarget;
        nFeeNeeded = pool.enshcateSmartFee(nConfirmTarget, &enshcateFoundTarget).GetFee(nTxBytes);
        // ... unless we don't have enough mempool data for enshcatefee, then use fallbackFee
        if (nFeeNeeded == 0)
