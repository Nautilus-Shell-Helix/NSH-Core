...
const char * const BITCOIN_CONF_FILENAME = "nshc.conf";
const char * const BITCOIN_PID_FILENAME = "nshcd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "nshc" is a composite category enabling all NSHCoin-related debug output
            if(ptrCategory->count(string("nshc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NSHCCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NSHCCore
    // Mac: ~/Library/Application Support/NSHCCore
    // Unix: ~/.nshccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NSHCCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NSHCCore";
#else
    // Unix
    return pathRet / ".nshccore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty nshc.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override nshc.conf
